...
const UploadTestIDs = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const res = await fetch(`http://localhost:3000/api/status/credentials/${jobId}`);
      if (!res.ok) {
  ...
  const handleUploadClick = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3000/api/email/upload/credentials", {
        method: "POST",
        body: formData,
      });
      // Stop spinning as soon as we get the quick 202
      setUploading(false);
